...
<div class="card text-center mx-auto" style="width: 250px;">
    ...
    <div class="card-body">
        <textarea name="texto" class="form-control" id="" cols="30" rows="6">{{ $nota->texto}}</textarea>
    </div>
    <div class="card-footer text-muted small">
        {{$nota->updated_at}}

        <a href="{{ URL::action('NotasController@edit', $nota->id) }}">
            <button type="submit" class="btn btn-info btn-sm float-right"><i class="far fa-save"></i></button>
        </a>

        <a href="{{ URL::action('NotasController@index') }}">
            <button type="button" class="btn btn-danger btn-sm float-right mr-1"><i
